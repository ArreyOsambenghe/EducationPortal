...
const ChatSideBar = ({conversations}:Props) => {
  ...
  const handleSubmit = async (e:FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("/api/agent/academic-structure", {
        method: "POST",
          ...
          if (log.role === "function-call" && log.message && log.name) {
                setChatMessages(prev => {
                  return prev.map(msg => {
                    ...
                    return {
                      ...msg,
                      task: [...existingTasks.filter(item  => item.name === 'thinking'), newTask],
                    };
                  });
                });
      }
  ...
  const handleCreateSession = async() => {
    ...
    const res = await createSession('ACADEMIC')
  ...
  return (
    <div className=' fixed top-0 right-0 w-86 h-full max-h-screen shadow-lg bg-white'>
      {/* Header */}
      <div className="flex items-center justify-between p-4 border-b border-gray-100">
        ...
        <button className='h-9 px-3 rounded-md hover:bg-gray-100 hover:text-gray-900' onClick={() => {}}>
          <X className="w-4 h-4" />
        </button>
      </div>
